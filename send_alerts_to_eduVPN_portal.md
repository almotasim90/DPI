...
Description: Ship EveBox alerts to eduVPN portal.
Category: DPI
---

This page is for sending EveBox alerts from the Events SQLite database into the EduVPN SQLite database. EveBox alerts are coming from `eve.json` file, which has all Suricata logs stored in. 
...
**Note** the `-D` parameter that tells EveBox where to store data files such as the file for the SQLite database. And the `--input` is where the `eve.json` file is located to be used as an input. 

By default EveBox runs as a server exposing a web interface on port `5636`, to see the result in the browser visit `http://localhost:5636`, Then you should see the events and alerts generated by Suricata. However, we are trying to get the results into the eduVPN portal using the SQLite database. 
...
Therefore, every SQLite database connection has the `main` database and also the `temp` database in case you deal with temporary database objects.

To attach an additional database to the current database connection, you can use the following statement inside the current SQLite database:
...
The EveBox database is storing the actual alert or event as JSON data, Therefore, an additional option is used in the statement to get those data. we are going to use `json_extract` option. 

**NOTE** You van read more about `json_extract` at this [page](https://samadhiweb.com/blog/2016.04.24.sqlite.json.html).

Since we are going to ship the alerts to eduVPN database `db.sqlite`, The best place to let each user see his own alerts is under user messages, So that in the eduVPN portal the user can see the alerts under `accounts` then `Events`.
...
**NOTE** It is important that each user gets his own alerts, without any conflict with other users. That is why we need `user_id` from the `connection_log` table  so that later we could insert the right data in the right place.


However, an IP address can be used by different users. So an additional filter is added to make sure that each user gets his own alerts at the certain time he was connected to that IP address. Also, we need to make sure that the user sees only alerts not any flow logs, so another filter was added. The result is this query:
...
By matching the `timestamp` with `connected_at` and `disconnected_at` time, we will make sure that alerts generated that time belongs to that user. So with this query you can get all the needed data to be inserted into the `user_messages` table.


### Extract other values

You can extract any value from the `eve.json` file. For example, `severity` alert number might be useful. So, by looking into the event from `eve.json` or EveBox SQLite database, You can see that the `severity` value is placed inside an alert array, in order to extract the value we could use the following query for example: 
...
This command will show all the alerts along with severity numbers, that can be used later to indicate the priority of the alert. 
...
So, now you do have already a query to be inserted into the `user_messages` table. You could easily insert a row into a table using data provided by a  `SELECT` statement. like the following statement: 
...
**HOWEVER** This kind of insertion will be automated. That means it will be inserted more than once upon a time. So, you have to make sure that inserted data are not getting duplicated. With considering this, another filter was added to make sure that the alerts inserted are not duplicated. You are going to use the `NOT_IN`  operator or similar. The end statement result will be this query:
...
That is the final query to insert the alerts into the `user_messages` table safely. 

Now, as a user you can go to the eduVPN portal, Then to `accounts` then under `Events` you should see the alerts. 



## Automate insertion with CRONTAB tool

CRONTAB tool is used for files used to schedule the execution of programs. You are going to schedule an insert command, to insert all the alerts into the
